from utils.checking_data_util import is_existed_user, is_blocked
from utils.db_util import query_db
from utils.date_util import convert_datetime_to_string
...
def show_list_messages(user_id):
    ...
    list_incoming_messages = query_db(query_incoming_message, is_fetching_data=True)
    ...
    if len(list_incoming_messages) > 0:
        ...
        for i in range(len(list_incoming_messages)):
            ...
            if list_incoming_messages[i]['sender_id'] not in list_ids:
                ...
                message['delivered_time'] = convert_datetime_to_string(list_incoming_messages[i]['delivered_time'],type_format=1)
                ...
                while j <= len(list_incoming_messages) - 2:
                    if list_incoming_messages[i]['sender_id'] == list_incoming_messages[j]['sender_id']:
                        ...
                        message['delivered_time'] = convert_datetime_to_string(list_incoming_messages[j]['delivered_time'],type_format=1)
    ...
    # all messages sent
    list_messages_sent = query_db(query_message_sent, is_fetching_data=True)
    if len(list_messages_sent) > 0:
        ...
        for i in range(len(list_messages_sent)):
            ...
            if list_messages_sent[i]['recipient_id'] not in list_ids:
                ...
                message['delivered_time'] = convert_datetime_to_string(list_messages_sent[i]['delivered_time'],type_format=1)
                ...
                while j <= len(list_messages_sent) - 2:
                    if list_messages_sent[i]['recipient_id'] == list_messages_sent[j]['recipient_id']:
                        ...
                        message['delivered_time'] = convert_datetime_to_string(list_messages_sent[j]['delivered_time'],type_format=1)
                        message['seen'] = list_messages_sent[j]['seen']

                        list_messages.append(message.copy())
                    j += 1

                info_message['sender_id'] = list_messages_sent[i]['recipient_id']
...
def show_detail_message(user_id, friend_id):
    ...
    list_data = query_db(query, is_fetching_data=True)
...
def sent_message(recipient_id, message, sender_id):
    ...
    if is_blocked(recipient_id, sender_id):
        print "You can not sent message, maybe You are block"
        return 0
...
def count_message_unread(user_id):
    ...
    number_messages_unread = query_db(query, is_fetching_data=True)
